...
for (i in 1:nrow(exprs)){
    ...
    filename_root <- paste0("../figs/synthetic_simul_thresh_", Xtype, "_pi", as.integer(pi1 * 10))
    
## Coverage
    plot_coverage <- res$coverage %>%
        filter(X == Xtype, pi == pi1,
               tauhat_type == "ra_db",
               sigmahat_type %in% c("HC2", "HC3")) %>%
        mutate(tau = factor(tau, levels = c(0, 0.025),
                            labels = c(FALSE, TRUE))) %>%
        select(-X, -tauhat_type, -pi) %>%
        group_by(resid, tau, sigmahat_type, exponent) %>%
