...
exprs <- expand.grid(thresh = 0.025,
                     pi1 = c(0.2, 0.5))

## Plots for bias
for (i in 1:nrow(exprs)){
    thresh <- as.numeric(exprs[i, 1])
    pi1 <- as.numeric(exprs[i, 2])
    
    filename_root <- paste0("../figs/synthetic_simul_thresh_t1_pi", pi1)

    ## Bias
    plot_bias <- res$bias %>% 
        filter(X == "t1", tau == thresh, pi == pi1) %>%
        select(-X, -tau, -pi) %>%
...
## Variance
    ...
    ggsave(filename = paste0(filename_root, "_sdinflate.pdf"), plot_sdinflate, width = 8, height = 3.3)
