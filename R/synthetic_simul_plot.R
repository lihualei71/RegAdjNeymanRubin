...
for (i in 1:nrow(exprs)){
    ...
    filename_root <- paste0("../figs/synthetic-simul-", Xtype, "-pi", pi1, "-rho", rh)
...
for (i in 1:nrow(exprs)){
    ...
    filename_root <- paste0("../figs/synthetic-simul-trim-", Xtype, "-pi", pi1, "-rho", rh)

    ## Bias
    plot_bias <- res$bias %>% 
        filter(X == Xtype, tau == 0.025, pi == pi1, rho == rh) %>%
        select(-X, -tau, -pi, -rho) %>%
        mutate(bias = pmin(bias, quantile(bias, 0.98))) %>%
        group_by(exponent, resid, tauhat_type) %>%
        summarize(bias = median(bias)) %>%
